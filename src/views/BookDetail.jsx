...
import { useBookByIdMutation } from "../service/api/books.js";
...
export const BookDetail = () => {
  ...
  const [useBook] = useBookByIdMutation();

  useEffect(() => {
    async function fetchData() {
      try {
        const response = await useBook(id).unwrap();
        setBook(response);
      } catch (error) {
        console.error(error);
      }
    ...
    <main className="flex-grow min-h-screen pt-24 container mx-auto px-4">
      <div className="bg-neutral-600 rounded-lg p-8 max-w-lg mx-auto shadow-xl">
        <div className="text-center">
          ...
          <button
            className="flex mx-auto w-40 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 items-center justify-center py-3 shadow-2xl"
            onClick={() => navigate(-1)}
          >
  ...
  const DetailView = () => (
    <main className="flex-grow min-h-screen pt-20 container mx-auto px-4 pb-8 max-w-6xl">
      <div className="bg-neutral-600 rounded-lg p-6 shadow-2xl">
        <button
          className="w-40 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 flex items-center justify-center gap-2 py-3 shadow-2xl mb-8"
          onClick={() => navigate(-1)}
        >
